...
### World & Rendering
- The game is a **top-down open world fantasy game** — rendered from a fixed overhead angle
- **All assets are algorithmic** — no art files; every visual is generated by code, either pre-computed or at runtime
- **Pseudo-3D appearance** — the world looks 3D but is rendered from a single fixed angle (normal maps, height-based shadows, parallax)
- **47-blob autotile tooling** — custom tooling to procedurally generate and pre-render all 47 autotile blob variants per terrain type
