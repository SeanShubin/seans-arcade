...
//!
//! Map layout loaded from maps/9keys.ron (generated by ldtk_convert from LDtk).
...
const MAP_RON: &str = "maps/9keys.ron";
...
fn load_map() -> GridMap {
    let ron_str = std::fs::read_to_string(MAP_RON)
        .unwrap_or_else(|e| panic!("Failed to read {MAP_RON}: {e}"));
    ron::from_str(&ron_str)
        .unwrap_or_else(|e| panic!("Failed to parse {MAP_RON}: {e}"))
}
